...
import (
	...
	"github.com/wso2-extensions/identity-tools-cli/iamctl/pkg/consents/piiCategories"
	"github.com/wso2-extensions/identity-tools-cli/iamctl/pkg/utils"
)

func ExportAll(exportFilePath string, format string) {

	log.Println("Exporting consent purposes...") //correct
	exportFilePath = filepath.Join(exportFilePath, utils.CONSENT_PURPOSES.String())

	if utils.IsResourceTypeExcluded(utils.CONSENT_PURPOSES) {
		return
	}

	err := piiCategories.ExportAll(exportFilePath, format)
