...
func ImportAll(inputDirPath string) {
	...
	importFilePath := filepath.Join(inputDirPath, utils.OIDC_SCOPES)
	...
	for _, file := range files {
		...
		if !utils.IsResourceExcluded(scopeName, utils.TOOL_CONFIGS.OidcScopeConfigs) {
			var scopeId string
			var err error
			scopeId, err = getScopeId(scopeFilePath, scopeName)

			if err != nil {
				log.Printf("Invalid file configurations for OIDC scope: %s. %s", scopeName, err)
			} else {
				err := importOidcScope(scopeId, scopeFilePath)
				if err != nil {
...
func importOidcScope(scopeId string, importFilePath string) error {
	...
	return updateScope(scopeId, importFilePath, modifiedFileData, fileInfo)
}
...
func getScopeId(scopeFilePath string, scopeName string) (string, error) {

	fileContent, err := ioutil.ReadFile(scopeFilePath)
	if err != nil {
		return "", fmt.Errorf("error when reading the file for OIDC scope: %s. %s", scopeName, err)
	}
	var scopeConfig oidcScopeConfig
	err = yaml.Unmarshal(fileContent, &scopeConfig)
	if err != nil {
		return "", fmt.Errorf("invalid file content for OIDC scope: %s. %s", scopeName, err)
	}
	existingScopeList, err := getOidcScopeList()
	if err != nil {
		return "", fmt.Errorf("error when retrieving the deployed OIDC scope list: %s", err)
	}
